...
func main() {
	...
	sha, err := fetchTagCommit(ctx, cfg.GitHubToken, latestVersion)
	if err != nil {
...
func fetchLatestVersion(ctx context.Context, token string) (string, error) {
	...
	if err := githubGet(ctx, token, "https://api.github.com/repos/ankitects/anki/releases/latest", &r); err != nil {
		return "", err
	}
...
func fetchTagCommit(ctx context.Context, token, tag string) (string, error) {
	...
	if err := githubGet(ctx, token, "https://api.github.com/repos/ankitects/anki/commits/"+tag, &c); err != nil {
		return "", err
	}
	return c.SHA, nil
}
